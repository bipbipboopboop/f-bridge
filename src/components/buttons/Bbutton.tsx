...
const Button: React.FC<ButtonProps> = ({ theme = "green", size = 3, disabled, children, className, ...props }) => {
  const buttonStyles = {
    base: "inline-flex items-center justify-center font-semibold rounded shadow-md transition-colors duration-300",
    size: {
      1: "px-2 py-1 text-xs sm:px-3 sm:py-2 sm:text-2xs md:px-4 md:py-2 md:text-base lg:px-5 lg:py-3 lg:text-sm",
      2: "px-3 py-2 text-sm sm:px-3 sm:py-2 sm:text-2xs md:px-6 md:py-4 md:text-lg lg:px-8 lg:py-5 lg:text-xl",
  ...
  const styles = `${buttonStyles.base} ${buttonStyles.size[size]} ${buttonStyles.color[theme]} ${className}`;
