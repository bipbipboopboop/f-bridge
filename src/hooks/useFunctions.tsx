...
const useFunctions = () => {
  ...
  const [createPlayerProfile, l11, e11] = useHttpsCallable<UserInfo, PlayerProfile>(functions, "createPlayerProfile");
  const [deleteAnonymousPlayer, l5, e5] = useHttpsCallable(functions, "deleteAnonymousPlayer");

  /**
   * GAME ROOM API
   */
  const [createGameRoom, l6, e6] = useHttpsCallable<void, GameState>(functions, "createGameRoom");
  const [joinGameRoom, l7, e7] = useHttpsCallable<string, void>(functions, "joinGameRoom");
  ...
  const isLoading = l0 || l1 || l2 || l3 || l4 || l5 || l6 || l7 || l8 || l9 || l10 || l11;

  const error = e0 || e1 || e2 || e3 || e4 || e5 || e6 || e7 || e8 || e9 || e10 || e11;
