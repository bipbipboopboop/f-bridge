...
export const createGameRoom = functions.https.onCall(async (data: void, context) => {
  ...
  // Create a new game room
  const gameRoomRef = admin.firestore().collection("gameRooms").doc(gameRoomData.roomID);
...
export const joinGameRoom = functions.https.onCall(async (roomID: string, context) => {
  ...
  if (gameRoomData.settings.isInviteOnly && !gameRoomData.invitedID.includes(context.auth.uid)) {
...
export const startGame = functions.https.onCall(async (data: void, context) => {
  ...
  if (gameRoomData.hostID !== context.auth!.uid) {
    throw new functions.https.HttpsError("failed-precondition", "Player is not the host.");
  }
  ...
  const readyPlayers = gameRoomData.players.filter((player: LobbyPlayerProfile) => player.isReady);
  if (readyPlayers.length != 4) {
    throw new functions.https.HttpsError("failed-precondition", "Insufficient ready players to start the game.");
  }
...
export const invitePlayer = functions.https.onCall(async (inviteeID: string, context) => {
  ...
  if (!inviterProfile) {
    throw new functions.https.HttpsError("not-found", "Inviter does not exist.");
  }

  // Check if inviter is in a room
  if (!inviterProfile.roomID) {
    throw new functions.https.HttpsError("failed-precondition", "Inviter is not in a room.");
  }
  ...
  if (isAlreadyInvited) {
    throw new functions.https.HttpsError("already-exists", "Invitee is already invited.");
  }
...
export const kickPlayer = functions.https.onCall(async (targetPlayerID: string, context) => {
  ...
  const targetPlayerRef = admin.firestore().collection("players").doc(targetPlayerID);

  const targetPlayerSnapshot = (await targetPlayerRef.get()) as DocumentSnapshot<PlayerProfile>;
  const targetPlayer = targetPlayerSnapshot.data();

  if (!targetPlayer) {
    throw new functions.https.HttpsError("not-found", "Player does not exist.");
  }

  if (!targetPlayer.roomID) {
    throw new functions.https.HttpsError("failed-precondition", "Player is not in a room.");
  }
  ...
  if (!gameRoom) {
    throw new functions.https.HttpsError("failed-precondition", "Room does not exist.");
  }

  if (gameRoom.status !== "Waiting") {
    throw new functions.https.HttpsError("failed-precondition", "The game has already started or ended.");
  }

  // Check if the player is the host
  if (gameRoom.hostID !== kickerID) {
    throw new functions.https.HttpsError("permission-denied", "You are not the host of the game.");
  }

  // Check if the target player is in the room
  if (targetPlayer.roomID !== targetPlayer.roomID) {
    throw new functions.https.HttpsError("failed-precondition", "Target player is not in the room.");
  }
