...
export const placeBid = functions.https.onCall(async (bid: Bid, context) => {
  ...
  const updatedGameRoom = produce(gameRoom, (gameRoom) => {
    ...
    if (bid.isPass) {
      ...
      biddingPhase.bidHistory.at(-1)![
        `p${gameRoomPlayer.position}` as "p0" | "p1" | "p2" | "p3"
      ].bid = bid;
    ...
    biddingPhase.currentPlayerIndex = ((biddingPhase.currentPlayerIndex + 1) %
      4) as 0 | 1 | 2 | 3;
  });
...
export const chooseTeammate = functions.https.onCall(
  async (card: Card, context) => {
    ...
    if (gameRoom.status !== "Choosing Teammate") {
      throw new functions.https.HttpsError(
        "failed-precondition",
        "You can't choose your teammate now."
      );
    }
    ...
    if (!gameRoom.biddingPhase) {
      throw new functions.https.HttpsError(
        "failed-precondition",
        "Can't find bidding phase."
      );
    }

    // 2. Check if the player is in the room
    const gamePlayerListRef = gameRoomRef.collection(
      "players"
    ) as CollectionReference<GamePlayer>;
    ...
    const gamePlayer = gamePlayerList.find(
      (gamePlayer) => gamePlayer.id === context.auth!.uid
    );
    ...
    if (gamePlayer.position !== gameRoom.biddingPhase!.currentPlayerIndex) {
      throw new functions.https.HttpsError(
        "failed-precondition",
        "It's not your turn to choose your teammate."
      );
    }
    ...
    if (gamePlayer.cards.includes(card)) {
      throw new functions.https.HttpsError(
        "failed-precondition",
        "You can't choose your own card!"
      );
    }
    ...
    const teammate = gamePlayerList.find(
      (gamePlayer) =>
        gamePlayer.cards.findIndex(
          (gamePlayerCard) =>
            gamePlayerCard.suit === card.suit &&
            gamePlayerCard.value === card.value
        ) !== -1
    )!;

    // Find the remaining 2 players
    const otherPlayers = gamePlayerList.filter(
      (plyr) => plyr.id !== teammate.id && plyr.id !== gamePlayer.id
    );
    ...
    (await gamePlayerListRef.listDocuments()).forEach(async (doc) => {
      ...
      const isDefenderTeam =
        doc.id === otherPlayers[0].id || doc.id === otherPlayers[1].id;
    ...
    const updatedGameRoom = produce(gameRoom, (gameRoom) => {
      ...
      const leadPlayerIndex = ((gameRoom.biddingPhase!.currentPlayerIndex + 1) %
        4) as 0 | 1 | 2 | 3;
    ...
    await gameRoomRef
      ...
      .doc("hiddenTrickPhase")
      .set(hiddenTrickPhase);
